export default function ProjectCard({ image, title, description, delay = 0 }) {
  return (
    <div
    ...
    >
      <img
        src={image}
        alt={title}
        className="rounded-md mb-4 w-full h-40 object-cover transition-transform duration-300 group-hover:scale-110 animate-fade-in"
        style={{ animationDelay: `${delay + 0.1}s`, animationFillMode: 'both' }}
      />
      <h3 className="text-xl font-bold mb-2  group-hover:text-primary/80 transition-colors duration-300">{title}</h3>
